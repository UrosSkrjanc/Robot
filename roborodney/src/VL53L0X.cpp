...
// VL53L0X datasheet.
#include "ros/ros.h"
#include <sys/time.h>	//millis() generated by 'gettimeofday' in time.h 
#include <stdio.h>	//stderr
#include <fcntl.h>	//#define O_RDWR 0x0002
...
{
  uint8_t buf[3];
  buf[0] = (value >> 24) & 0xFF; // value highest byte
  buf[1] = (value >> 16) & 0xFF;
  buf[2] = (value >>  8) & 0xFF;
  buf[3] =  value        & 0xFF; // value lowest byte
  
  //last_status = ?
  i2c_smbus_write_block_data(fd,reg,4,buf);
}

// Read an 8-bit register
uint8_t VL53L0X::readReg(uint8_t reg)
{
  uint8_t value = i2c_smbus_read_byte_data(fd,reg);
  //last_status = ?
  return value;
...
//  uint16_t value;
  ...
  //last_status = ?
//  value  = (uint16_t)readReg(reg)   << 8; // value high byte
//  value |=           readReg(reg+1);      // value low byte
...
uint32_t VL53L0X::readReg32Bit(uint8_t reg)
{
  uint8_t value[4];
  uint32_t buf;
  i2c_smbus_read_block_data(fd,reg,value);
  //last_status = ?
  buf  = (uint32_t)value[0] << 24; // value highest byte
  buf |= (uint32_t)value[1] << 16;
  buf |= (uint16_t)value[2] <<  8;
  buf |=           value[3];       // value lowest byte
  return buf;
}
...
void VL53L0X::writeMulti(uint8_t reg, uint8_t const * src, uint8_t count)
{
  ...
  //last_status = ?
  i2c_smbus_write_block_data(fd,reg,count,buf);
}
...
void VL53L0X::readMulti(uint8_t reg, uint8_t * dst, uint8_t count)
{
  uint8_t buf[count];
  uint8_t i=count;
/*  i2c_smbus_read_block_data(fd,reg,buf);
  while (i-- > 0)
  {
    *(dst++) = buf[count-i];
  }
...
//  uint16_t range = readReg16Bit(RESULT_RANGE_STATUS + 10);
  uint16_t range = readReg(RESULT_RANGE_STATUS + 10) << 8;
